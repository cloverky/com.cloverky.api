...
_FEATURE_COLS = ["Pclass", "gender", "SibSp", "Parch", "Embarked", "Title", "AgeGroup", "FareBand", "Deck"]

_AGE_LABELS = {
    0: "Unknown", 1: "Baby (0~5세)", 2: "Child (6~12세)", 3: "Teen (13~18세)",
    4: "Student (19~24세)", 5: "Young Adult (25~35세)", 6: "Adult (36~60세)", 7: "Senior (61세+)",
}

_FEAT_LABELS = {
    "Pclass": "PClass(티켓 등급)", "gender": "Gender(성별)",
    "SibSp": "SibSp(형제·배우자 수)", "Parch": "Parch(부모·자녀 수)",
    "Embarked": "Embarkation(탑승항)", "Title": "Title(호칭)",
    "AgeGroup": "Age(나이)", "FareBand": "Fare(운임)", "Deck": "Cabin(객실 구역)",
}
...
class SmithCaptainInteractor(SmithCaptainUseCase):
    ...
    async def chat(self, schema: ChatSchema) -> SmithChatResponse:
        ...
        if intent == "STATISTICS":
            if any(w in question for w in ["몇명", "몇 명", "몇명이나", "몇명이", "몇 명이", "명이나", "인원", "탑승객 수"]):
        ...
        return SmithChatResponse(reply=(
    ...
    def _engineer(self, raw: DataFrame) -> DataFrame:
        ...
        d["Survived"] = pd.to_numeric(d["Survived"], errors="coerce").fillna(0).astype(int)

        d["Title"] = (
            d["Name"].str.extract(r"([A-Za-z]+)\.", expand=False)
            .replace(["Capt", "Col", "Don", "Dr", "Major", "Rev", "Jonkheer", "Dona", "Mme"], "Rare")
            .replace(["Countess", "Lady", "Sir"], "Royal")
            .replace({"Mlle": "Mr", "Ms": "Miss"})
        )
        d["Title"] = d["Title"].map({"Mr": 1, "Miss": 2, "Mrs": 3, "Master": 4, "Royal": 5, "Rare": 6}).fillna(0).astype(int)
        d["gender"] = d["gender"].map({"male": 0, "female": 1})
        d["Age"] = d["Age"].fillna(-0.5)
        d["AgeGroup"] = (
            pd.cut(d["Age"], bins=[-1, 0, 5, 12, 18, 24, 35, 60, np.inf], labels=[0, 1, 2, 3, 4, 5, 6, 7])
            .astype("Int64").fillna(0).astype(int)
        )
        d["Embarked"] = d["Embarked"].fillna("S").map({"S": 1, "C": 2, "Q": 3})
        d["FareBand"] = pd.qcut(d["Fare"], 4, labels=[1, 2, 3, 4], duplicates="drop").fillna(1).astype(int)
        _deck = {"A": 1, "B": 2, "C": 3, "D": 4, "E": 5, "F": 6, "G": 7, "T": 8}
        d["Deck"] = d["Cabin"].str.extract(r"^([A-Z])", expand=False).map(_deck).fillna(0).astype(int)
        return d.drop(columns=["Name", "Age", "Fare", "Ticket", "Cabin", "PassengerId"], errors="ignore")
    ...
    def _answer_breakdown(self, train_set: DataFrame, question: str) -> str:
        ...
        for idx, row in grouped.sort_index().iterrows():
            lines.append(f"  {label_fn(idx)}: {row['mean'] * 100:.1f}%  ({int(row['count'])}명)")
        return "\n".join(lines)
    ...
    async def _answer_count(self, train_set: DataFrame) -> str:
        total = await self.walter.get_total_count()
        survived_series = pd.to_numeric(train_set.get("Survived", pd.Series(dtype=float)), errors="coerce").dropna()
    ...
    def _answer_prediction(self, train_set: DataFrame, question: str) -> str:
        ...
        result = "생존했을 가능성이 높습니다" if pred == 1 else "생존하지 못했을 가능성이 높습니다"
