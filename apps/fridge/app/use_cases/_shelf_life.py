...
def expiry_from_purchase(name: str, purchased: date, storage: str = "냉장") -> date:
...
def shelf_life_hint(item: InventoryStatusItem) -> int | None:
...
def compute_status(item: InventoryStatusItem, today: date | None = None) -> str:
...
def count_expiring_soon(items: list[InventoryStatusItem], today: date | None = None) -> int:
    today = today or date.today()
...
def count_low_stock(items: list[InventoryStatusItem]) -> int:
