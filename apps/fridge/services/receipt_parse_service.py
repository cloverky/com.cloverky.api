"""영수증 이미지 → JSON (Gemini). 이미지는 메모리에서만 사용."""
...
from fridge.schemas.receipt_schema import ReceiptLineParsed, ReceiptParseResult
...
def parse_receipt_image(image_bytes: bytes, mime_type: str) -> ReceiptParseResult:
    ...
    for row in data.get("items") or []:
        ...
        items.append(ReceiptLineParsed(name=name, quantity=max(1, qty), unit=unit))
